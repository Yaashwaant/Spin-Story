  ...
  try {
    ...
    if (demoUsers.length < 2) {
      ...
      return NextResponse.json({ 
        error: "Not enough demo users found",
    ...
    return NextResponse.json({ 
      success: true,
      message: "Demo wardrobe seeding completed successfully!",
      users: [
        { id: demoUsers[0].id, name: demoUsers[0].name, items: user1Items.length },
        { id: demoUsers[1].id, name: demoUsers[1].name, items: user2Items.length }
      ]
    })
    ...
    return NextResponse.json({ 
      error: "Failed to seed demo wardrobe",
